...
export const treeTablePT = {
    ...
    headerrow: ({ props }) => ({
        className: classNames({
            'flex flex-nowrap w-full text-slate-700': props.scrollable
        })
    }),
    // row: ({ context }) => ({
    //     className: classNames(
    ...
    row: ({ context }) => ({
        className: classNames(
            context.selected ? 'bg-blue-50' : 'bg-white',
        )
    }),
    column: {
        headercell: ({ context }) => ({
        ...
        sorticon: ({ context }) => ({
            className: classNames('ml-2', {
                'text-blue-700': context.sorted,
                'text-slate-700': !context.sorted
            })
        }),
        sortbadge: {
            className: classNames(
        ...
        checkboxicon: ({ context }) => ({
            className: classNames('w-4 h-4 transition-all duration-200 text-base', {
